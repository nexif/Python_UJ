...
class Rectangle: #Klasa reprezentująca prostokąty na płaszczyźnie

    def __init__(self, x1, y1, x2, y2): #walidacja x1 < x2, y1 < y2
        if x1 >= x2 or y1 >= y2: raise ValueError("prawidłowe dane powinny spełniać warunek x1 < x2, y1 < y2")

        self.pt1 = Point(x1, y1)
        self.pt2 = Point(x2, y2)

    def __str__(self): #Zwraca string '[(x1, y1), (x2, y2)]'
        return "[({0}, {1}), ({2}, {3})]".format(self.pt1.x, self.pt1.y, self.pt2.x, self.pt2.y)

    def __repr__(self): #Zwraca string "Rectangle(x1, y1, x2, y2)
